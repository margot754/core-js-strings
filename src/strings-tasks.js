...
function isPalindrome(str) {
  const formStr = str.toLowerCase().replace(/[\W]/g, '');
  return formStr.split('').reverse().join('') === formStr;
}
...
function getCardId(value) {
  const deckRanks = 'A234567891JQK';
  const deckSuits = '♣♦♥♠';
  const rank = value.slice(0, 1);
  const suit = value.slice(-1);

  return deckSuits.indexOf(suit) * deckRanks.length + deckRanks.indexOf(rank);
}
